-- 1)
SELECT pls14.stabr,
 sum(pls14.gpterms) AS comps14,
 sum(pls09.gpterms) AS comps09,
 round( (CAST(sum(pls14.gpterms) AS decimal(10,1)) - sum(pls09.gpterms)) /
 sum(pls09.gpterms) * 100, 2 ) AS comps_pct_change,
 
 sum(pls14.pitusr) AS comp_user14,
 sum(pls09.pitusr) AS comp_user09,
  round( (CAST(sum(pls14.pitusr) AS decimal(10,1)) - sum(pls09.pitusr)) /
 sum(pls09.pitusr) * 100, 2 ) AS users_pct_change
...
ORDER BY comps_pct_change DESC ;
